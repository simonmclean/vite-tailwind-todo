...
export default function({ todoItems, onToggleDone, onEdit, onDelete }: TodoListProps) {
  // Sort by:
  // - lastUpdatedAt
  // - createdAt
  // - id
    ...
    <ol className="list-none px-0">
      {todoItemsSorted.map(item =>
        <li className="mb-6 p-0" key={item.id} id={`todo-list-item-${item.id}`}>
          <TodoItemComponent item={item} onDelete={onDelete} onToggleDone={onToggleDone} onEdit={onEdit} />
        </li>
      )}
    </ol>
